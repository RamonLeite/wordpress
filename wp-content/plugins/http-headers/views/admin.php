	...
	<section class="hh-panel">
		...
		<form method="post" action="options.php">
			...
			<tbody>
				...
				<td>
					<select name="hh_strict_transport_security_value" class="http-header-value"<?php disabled($strict_transport_security, 0); ?>>
					<?php
					$items = array('max-age=31536000', 'max-age=31536000; includeSubDomains');
					$strict_transport_security_value = get_option('hh_strict_transport_security_value');
					foreach ($items as $item) {
						?><option value="<?php echo $item; ?>"<?php selected($strict_transport_security_value, $item); ?>><?php echo $item; ?></option><?php
					}
					?>
					</select>
				</td>
